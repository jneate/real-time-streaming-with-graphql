...
class DemoSankeyLayout<K1, K2, D1, D2> {
  _ctx: DvtDiagramLayoutContext<K1, K2, D1, D2>;

  constructor(ctx: DvtDiagramLayoutContext<K1, K2, D1, D2>) {
    this._ctx = ctx;
  }

  static COLUMN_WIDTH = 1000;
  static ROW_GAP = 15;
  static LABEL_GAP = 20;
  static LINK_CURVATURE = 0.33;
   ...
   */
  static layout = <K1, K2, D1, D2>(
    ctx: DvtDiagramLayoutContext<K1, K2, D1, D2>
  ) => {
   ...
   */
  _positionColumn = (columns: K1[][], columnIndex: number) => {
   ...
   */
  _iterateNodePositions = (columns: K1[][], upstream: boolean) => {
   ...
   */
  _separateNodes = (columns: K1[][], columnIndex: number) => {
    const ctx = this._ctx;
  ...
  _getColumns = () => {
    ...
    for (let i = 0; i < nodeCount; i++) {
      ...
      const nodeData = this._ctx.getNodeByIndex(i).getData();
    ...
    const columnCount = (maxDistance || 1) + 1;
